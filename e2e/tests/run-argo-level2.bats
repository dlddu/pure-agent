...
@test "verify_cycle: calls S3 transcript assertions when S3_ENDPOINT_URL is set" {
  # Arrange
  assert_workflow_succeeded() { return 0; }
  export -f assert_workflow_succeeded

  local s3_transcript_called=false
  local s3_planner_called=false
  ...
  assert_s3_planner_transcript_exists() {
    echo "assert_s3_planner_transcript_exists called" >> "$WORK_DIR/s3-calls.txt"
    return 0
  }
  export -f assert_s3_transcript_exists assert_s3_planner_transcript_exists
  ...
  # Assert — both S3 assertions should have been called
  grep -q "assert_s3_transcript_exists called" "$WORK_DIR/s3-calls.txt"
  grep -q "assert_s3_planner_transcript_exists called" "$WORK_DIR/s3-calls.txt"
}
...
@test "verify_cycle: fails when S3 planner transcript assertion fails" {
  # Arrange
  assert_workflow_succeeded() { return 0; }
  export -f assert_workflow_succeeded

  assert_s3_transcript_exists() { return 0; }
  assert_s3_planner_transcript_exists() { return 1; }
  export -f assert_s3_transcript_exists assert_s3_planner_transcript_exists
  export S3_ENDPOINT_URL="http://localstack:4566"
  ...
  # Act
  run verify_cycle "$yaml_file" "failing-planner-s3-wf" 0
