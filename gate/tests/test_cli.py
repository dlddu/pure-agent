"""Tests for gate.cli -- CLI argument parsing, orchestration, error handling."""
...
from gate.cli import _write_fallback_output, main, run
...
class TestWriteFallbackOutput:
    def test_writes_false_when_output_arg_present(self, tmp_path, monkeypatch, caplog):
        ...
        monkeypatch.setattr(sys, "argv", ["gate", "--output", out])
        with caplog.at_level(logging.INFO, logger="gate"):
            _write_fallback_output()
    ...
    @pytest.mark.parametrize(
        "argv",
        [
            pytest.param(["gate", "--depth", "0"], id="no-output-arg"),
            pytest.param(["gate", "--output"], id="output-is-last-arg"),
            pytest.param(["gate", "--output", ""], id="empty-string-output"),
        ],
    )
    ...
    def test_silently_returns_when_path_unwritable(self, monkeypatch, caplog):
        ...
        monkeypatch.setattr(sys, "argv", ["gate", "--output", bad_path])
        with caplog.at_level(logging.INFO, logger="gate"):
            _write_fallback_output()
        assert "fallback" not in caplog.text
...
class TestRun:
    def test_crash_exits_with_code_1(self, crash_env, caplog):
        ...
        assert rec.getMessage() == "Gate crashed with unhandled exception"
    ...
    def test_systemexit_propagates(self, work_env, monkeypatch, caplog):
        ...
        monkeypatch.setattr(
            sys, "argv", ["gate", "--output", output_path]
