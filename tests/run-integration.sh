#!/usr/bin/env bash
# tests/run-integration.sh — Integration E2E 테스트 러너 (kind + Argo, mock 기반)
#
# Mock:
...
#
# Usage:
#   ./tests/run-integration.sh [--scenario <name|all>] [--namespace <ns>] [--context <ctx>]
#
...
#   MOCK_AGENT_IMAGE  — mock-agent 이미지 (기본값: ghcr.io/dlddu/pure-agent/tests/mock-agent:latest)
...
MOCK_AGENT_IMAGE="${MOCK_AGENT_IMAGE:-ghcr.io/dlddu/pure-agent/tests/mock-agent:latest}"
