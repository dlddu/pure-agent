...
load_run_argo() {
  ...
  export SCENARIOS_DIR="${SCENARIOS_DIR:-${TESTS_DIR}/scenarios}"
  ...
  export MOCK_AGENT_IMAGE="${MOCK_AGENT_IMAGE:-ghcr.io/dlddu/pure-agent/tests/mock-agent:latest}"
  ...
  # shellcheck disable=SC1090
  source "$TESTS_DIR/run-integration.sh" --source-only
}
