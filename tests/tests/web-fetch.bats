...
# tests/tests/web-fetch.bats — Gatekeeper 통합 테스트: web_fetch_get 승인/거절/만료 시나리오
#
# DLD-780: web_fetch_get e2e 테스트 (skip 상태)
# DLD-781: skip 제거 후 실제 Gatekeeper 구현과 함께 활성화
#
# 테스트 구조:
#   - Docker Compose 환경에서 실행됩니다 (gatekeeper 서비스 포함).
#   - 각 테스트는 하나의 web_fetch_get 시나리오만 검증합니다.
#   - mock-agent가 web_fetch_get 호출 → Gatekeeper 승인 요청 생성
#   - e2e 스크립트가 Gatekeeper API를 통해 승인/거절/만료 처리
#   - web_fetch_get 폴링 결과를 검증합니다.
#
# 전제 조건 (skip 제거 시 필요):
#   - docker (compose plugin 포함)
#   - curl, jq, yq
#   - tests/docker-compose.yml (gatekeeper 서비스 정의 포함)
#   - GATEKEEPER_URL 환경 변수 설정
#
# 실행 방법:
#   bats tests/tests/web-fetch.bats
#
# 개별 테스트 실행:
#   bats tests/tests/web-fetch.bats --filter "approved"
...
setup() {
  ...
  # shellcheck disable=SC1090
  source "${TESTS_DIR}/lib/gatekeeper.sh" --source-only
}
...
# ═══════════════════════════════════════════════════════════════════════════════
# 테스트 1: web_fetch_get 승인 → 정상 fetch
#
# 검증 항목:
#   - mock-agent가 web_fetch_get 호출 → Gatekeeper에 PENDING 요청 생성
#   - e2e 스크립트가 테스트 사용자로 로그인하여 JWT 획득
#   - PATCH /api/requests/:id/approve 호출로 승인 처리
#   - web_fetch_get 폴링이 승인을 감지하고 HTTP fetch 수행
#   - mock-agent 결과: 정상 응답 (exit 0)
# ═══════════════════════════════════════════════════════════════════════════════

@test "web_fetch_get: approved — gatekeeper approval triggers successful http fetch" {
  # Arrange
  ...
  # web_fetch_get 폴링이 승인을 감지하고 fetch를 완료할 때까지 대기
  # (export-handler가 web_fetch_get 결과를 처리)
  local eh_exit=0
  run_export_handler || eh_exit=$?

  # Assert: export-handler 정상 종료 (web_fetch_get 성공)
  [ "$eh_exit" -eq 0 ]
...
# ═══════════════════════════════════════════════════════════════════════════════
# 테스트 2: web_fetch_get 거절 → 에러 반환
#
# 검증 항목:
#   - mock-agent가 web_fetch_get 호출 → Gatekeeper에 PENDING 요청 생성
#   - e2e 스크립트가 PATCH /api/requests/:id/reject 호출로 거절 처리
#   - web_fetch_get 폴링이 거절을 감지하고 에러를 반환
#   - mock-agent 결과: 에러 응답 (비정상 종료 또는 에러 메시지 포함)
# ═══════════════════════════════════════════════════════════════════════════════

@test "web_fetch_get: rejected — gatekeeper rejection causes web_fetch_get to return error" {
  # Arrange
  ...
  # web_fetch_get 폴링이 거절을 감지하고 에러를 반환할 때까지 대기
  local eh_exit=0
  run_export_handler || eh_exit=$?

  # Assert: web_fetch_get 거절로 인한 에러가 mock-agent 출력에 반영됨
  # (export-handler는 web_fetch_get 에러를 처리하고 정상 종료할 수 있음)
  # 실제 에러 검증 방식은 DLD-781에서 구현에 맞게 조정
  [[ "$output" == *"rejected"* ]] || [[ "$output" == *"error"* ]] || [ "$eh_exit" -ne 0 ]
}

# ═══════════════════════════════════════════════════════════════════════════════
# 테스트 3: web_fetch_get 만료 → 에러 반환
#
# 검증 항목:
#   - timeoutSeconds를 매우 짧게 설정 (예: 1초)
#   - mock-agent가 web_fetch_get 호출 → Gatekeeper에 PENDING 요청 생성
#   - 승인 처리 전에 timeoutSeconds 경과 → 요청 만료
#   - web_fetch_get 폴링이 만료를 감지하고 에러를 반환
#   - mock-agent 결과: timeout 에러 응답
# ═══════════════════════════════════════════════════════════════════════════════

@test "web_fetch_get: expired — request timeout causes web_fetch_get to return error" {
  # Arrange
  ...
  # export-handler 실행: web_fetch_get 폴링이 만료를 감지하고 에러 반환
  local eh_exit=0
  run_export_handler || eh_exit=$?

  # Assert: web_fetch_get timeout 에러가 출력에 반영됨
  # 실제 에러 검증 방식은 DLD-781에서 구현에 맞게 조정
